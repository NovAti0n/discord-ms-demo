...
export async function getOAuthToken(code) {
	...
	if (response.ok) {
		...
		return data;
	} else throw new Error(`Unable to fetch Microsoft OAuth token: [${response.status}] ${response.statusText}`);
}
...
export async function getUserData(token) {
	...
	if (response.ok) {
		const data = await response.json();
		return data;
	} else throw new Error(`Unable to fetch Microsoft user data: [${response.status}] ${response.statusText}`);
}
